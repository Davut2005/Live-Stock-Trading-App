...
// Copy constructor
HealthStock::HealthStock(const HealthStock &other) : Stock(other) {}

// Destructor
HealthStock::~HealthStock() = default;

// getType implementation
std::string HealthStock::getType() const { return "Health"; }
...
void HealthStock::display() const {
  std::cout << getType() << " Stock: " << name << " | Price: " << price
            << " | Quantity: " << quantity << " | Value: " << totalValue()
            << '\n';
}
